...
function DataCell({ value, tooltip }: { value: CellValue; tooltip?: string }) {
  const content = (() => {
    if (value === 'yes') {
      return (
        <>
          <Check size={20} strokeWidth={2} className="text-success mx-auto" aria-hidden="true" />
    ...
    if (value === 'no') {
      return (
        <>
          <Minus size={20} strokeWidth={2} className="text-neutral-300 mx-auto" aria-hidden="true" />
    ...
    if (value === 'partial') {
      return (
        <>
          <Minus size={20} strokeWidth={2} className="text-warning mx-auto" aria-hidden="true" />
          <span className="sr-only">Cobertura parcial</span>
        </>
      )
    }
    return <span className="font-mono text-sm font-medium text-neutral-900 tabular-nums">{value}</span>
  })()

  if (!tooltip) {
    return <div className="flex items-center justify-center py-3 px-4">{content}</div>
  }
        ...
        <TooltipTrigger asChild>
          ...
          >
            ...
            {(value === 'partial' || typeof value === 'string') && (
              <Info size={12} className="text-neutral-400 hover:text-primary-700 transition-colors duration-150" aria-hidden="true" />
            )}
          </button>
        </TooltipTrigger>
      ...
      <div className="max-w-[1200px] mx-auto px-6 lg:px-16">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-lg text-neutral-500 mt-3">
            Revisá exactamente qué incluye y qué no incluye cada plan antes de cotizar.
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-elevation-1 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[600px]">
              <caption className="sr-only">Comparación de coberturas de {insurer.name}</caption>
              <thead>
                  ...
                  {planNames.map((name, i) => (
                    <th
                      ...
                      className={[
                        ...
                        i === 1 ? 'bg-primary-50 border-x border-primary-200' : '',
                      ].join(' ')}
                    >
                      {i === 1 && (
