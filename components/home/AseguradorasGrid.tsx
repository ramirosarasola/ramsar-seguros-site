...
const INSURERS = [
  {
    ...
    rating: 4.4,
    reviews: 312,
    bullets: ['Granizo + robo total', 'Auxilio 24 h en todo el país', 'Sin franquicia en luneta'],
  ...
  },
  {
    ...
    rating: 4.7,
    reviews: 541,
    bullets: ['Cobertura nacional + MERCOSUR', 'Cotización 100% digital', 'App con denuncia online'],
  ...
  },
  {
    ...
    rating: 4.5,
    reviews: 187,
    bullets: ['Especialista en alta gama', 'Tasación con app', 'Servicio premium'],
  ...
  },
  {
    ...
    rating: 4.2,
    reviews: 296,
    bullets: ['Grupo internacional', 'Cobertura todo riesgo flexible', 'Atención telefónica 24/7'],
  ...
  },
  {
    ...
    rating: 4.6,
    reviews: 412,
    bullets: ['Cooperativa argentina', 'Sin sorpresas en la prima', 'Tasa de siniestros baja'],
...
function StarRating({ value }: { value: number }) {
  return (
    <span className="font-mono text-[12px] text-accent-500 flex items-center gap-0.5" aria-label={`${value} estrellas`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          ...
          className={i < Math.floor(value) ? 'text-accent-500' : 'text-neutral-300'}
        />
      ))}
    </span>
  )
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="font-sans text-[16px] text-neutral-500 mt-2 max-w-[56ch]">
              Comparamos precios, coberturas y opiniones reales para que tomes la mejor decisión.
            </p>
          </div>
          <Link
            href="/aseguradoras"
            className="font-sans font-semibold text-[14px] text-primary-700 no-underline hover:underline hover:underline-offset-2 flex-shrink-0"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {INSURERS.map((ins) => (
            ...
            >
              ...
              <h3 className="font-sans font-semibold text-[15px] text-neutral-900 group-hover:text-primary-700 transition-colors duration-[120ms]">
                {ins.name}
              </h3>
              ...
              <ul className="flex flex-col gap-1 mb-3" aria-label={`Características de ${ins.name}`}>
                {ins.bullets.map((b) => (
                  <li key={b} className="font-sans text-[13px] text-neutral-600 flex items-start gap-1.5">
                    <span className="text-primary-700 font-bold flex-shrink-0 mt-px">✓</span>
                    {b}
                  </li>
                ))}
              </ul>

              <p className="font-mono text-[13px] text-neutral-900 font-medium">{ins.price}</p>
            </Link>
          ))}
        </div>
