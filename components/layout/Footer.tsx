...
function FooterColHeading({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="font-sans font-semibold text-[11px] tracking-widest uppercase text-white/50 mb-3">
      {children}
    </h2>
  );
}
...
function FooterLogo() {
  return (
    ...
    >
      <span
        className="relative self-center shrink-0 w-4 h-4 rounded-full bg-accent-500"
        aria-hidden="true"
      >
...
export function Footer() {
  return (
    <footer role="contentinfo" className="bg-secondary-700">
      ...
      <div className="max-w-300 mx-auto px-6 lg:px-16 pt-20 pb-12">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-[1.4fr_1fr_1fr_1fr_1fr] gap-8 lg:gap-8">
          ...
          <div className="col-span-2 md:col-span-3 lg:col-span-1 flex flex-col gap-4">
            ...
            <p className="font-sans text-[14px] text-white/78 leading-relaxed max-w-[28ch]">
              El amigo informado que te ayuda a elegir tu seguro de auto en
              Argentina.
            </p>
            {/* Social icons */}
            <div className="flex items-center gap-3 mt-1">
              {[
                {
                  Icon: Globe,
                  label: "Instagram de Ramsar Seguros",
                  href: "#",
                },
                { Icon: Music2, label: "TikTok de Ramsar Seguros", href: "#" },
                {
                  Icon: Briefcase,
                  label: "LinkedIn de Ramsar Seguros",
                  href: "#",
                },
                {
                  Icon: MessageCircle,
                  label: "WhatsApp de Ramsar Seguros",
                  href: "#",
                },
              ].map(({ Icon, label, href }) => (
                <a
                  ...
                  rel="me noopener"
                  target="_blank"
                  className="w-8 h-8 flex items-center justify-center text-white/70 hover:text-accent-500 transition-colors duration-120 rounded"
                >
      ...
      <div className="border-t border-b border-white/10">
        <div className="max-w-300 mx-auto px-6 lg:px-16 py-8">
      ...
      <div className="bg-secondary-900">
        <div className="max-w-300 mx-auto px-6 lg:px-16 py-5">
            ...
            <nav aria-label="Legal" className="flex items-center gap-1">
              {LEGAL_LINKS.map(({ href, label }, i) => (
                <span key={href} className="flex items-center">
                  {i > 0 && (
                    <span
                      className="font-sans text-[13px] text-white/30 px-2"
                      aria-hidden="true"
                    >
                      ·
                    </span>
                  ...
                  <Link
                    href={href}
                    className="font-sans text-[13px] text-white/70 no-underline hover:text-accent-500 transition-colors duration-120"
                  >
                    {label}
                  </Link>
                </span>
