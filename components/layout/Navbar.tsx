...
function RamsarLogo({ scrolled }: { scrolled: boolean }) {
  return (
    ...
    >
      ...
      <span
        className={[
          'relative self-center flex-shrink-0 rounded-full bg-accent-500',
          'transition-all duration-[220ms] ease-out',
          scrolled ? 'w-[14px] h-[14px]' : 'w-4 h-4',
        ].join(' ')}
        aria-hidden="true"
      >
    ...
    <>
      <header
        className={[
          ...
          scrolled
            ? 'h-16 bg-neutral-50/95 backdrop-blur-[12px] shadow-elevation-2'
            : 'h-[72px] bg-neutral-50 border-b border-neutral-200',
        ].join(' ')}
      >
        <nav
          aria-label="Principal"
          className="h-full max-w-[1200px] mx-auto px-6 lg:px-16 flex items-center justify-between"
        >
          {/* Left: Logo */}
              ...
              {NAV_LINKS.map(({ href, label }) => {
                ...
                return (
                  ...
                  >
                    ...
                    {active && (
                      <span
                        className="absolute left-0 right-0 h-[2px] bg-primary-700 rounded-full"
                        style={{ bottom: '-13px' }}
                        aria-hidden="true"
                      />
                    )}
